...
contract SnowballBenefit {
    ...
    modifier onlyOwner() {
        require(
            _owner == msg.sender,
            "Benefit: caller is not the owner address!"
        );
        _;
    }
    ...
    modifier onlyAdmin() {
        require(
            _admin == msg.sender || _owner == msg.sender,
            "Benefit: caller is not the admin address!"
        );
        _;
    }
    ...
    function getBenefitTypes() public pure returns (string memory) {
        string
            memory dataTypes = "Benefit(uint32 chainId,address nftContract,uint32 expiration,uint8 maxUsage,string content,address operator,uint256 deadline,uint256 nonce)";
        return dataTypes;
    }

    function getUsageTypes() public pure returns (string memory) {
        string
            memory dataTypes = "Usage(address user,uint32 benefitId,uint32 nftId,uint256 deadline,uint256 nonce)";
        return dataTypes;
    }

    function getRecordTypes() public pure returns (string memory) {
        string
            memory dataTypes = "Record(uint32 benefitId,address operator,uint256 deadline,uint256 nonce)";
        return dataTypes;
    }
    ...
    function _recordUsage(
        ...
        require(deadline >= block.timestamp, "Benefit: expired deadline");
        require(
            verifyUsageSig(user, benefitId, nftId, deadline, nonce, sig),
            "Benefit: user sig does not match"
        );

        Benefit memory benefit = benefits[benefitId];
        require(
            usageCount[benefitId][nftId] < benefit.maxUsage,
            "Benefit: already used max usage count"
        );

        _usageId++;
        usages[_usageId] = Usage(_usageId, benefitId, nftId, user);
    ...
    function recordUsage(
        ...
        Benefit memory benefit = benefits[benefitId];
        require(
            msg.sender == benefit.operator,
            "Benefit: sender is not the benefit operator"
        );
    ...
    function relayRecordUsage(
        ...
        Benefit memory benefit = benefits[benefitId];
        require(
            verifyRecordSig(benefitId, operator, opDeadline, opNonce, opSig),
            "Benefit: operator sig does not match"
        );
        require(
    ...
    function getAllBenefits(
        address nftAddr
    ) public view returns (Benefit[] memory) {
